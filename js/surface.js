...
// the outskirts: the outer band of the map where Understory dungeon-mouths may open
const OUTSKIRT_BAND=26;
const isOutskirts=(x,y)=>Math.min(x,y,W-1-x,H-1-y)<=OUTSKIRT_BAND;
...
function genWorld(){
 ...
 {
  const want=ri(3,4);
  // dungeon-mouths only open on the OUTSKIRTS — the wild edges of the map, well away
  // from where towns take root at the centre
  const cand=openTiles.filter(([x,y])=>{
   if(bld[idx(x,y)]>=0||nodeAt.has(idx(x,y)))return false;
   if(!isOutskirts(x,y))return false;
...
const SALVAGE_CAP=55;   // ground finds only get picked up by the Sage — cap them so
                        // uncollected treasure can't pile up and choke the render
function spawnFind(x,y,kind,amt,relic){
 salvage.push({x:x*TILE+TILE/2,y:y*TILE+TILE/2,t:R()*6.28,kind,amt:amt||0,relic:relic||null,born:simMin});
 if(salvage.length>SALVAGE_CAP){
  // sacrifice the oldest common find (gold/gem) first; keep rare relics longest
  let i=salvage.findIndex(s=>s.kind!=='relic'&&!s.relic);
  salvage.splice(i<0?0:i,1);
 }
}
...
function decayBuilding(b,stage,quiet){
 ...
 if(stage===1){
  b.decay=1;
  if(!quiet)tale([],'The '+decayLabel(b)+' emptied out and went dark — its halls abandoned to flickering, gutted ruin. Something moves in the dead floors now.',true);
 }else if(stage===2){
  b.decay=2;
  if(!quiet)tale([],'The wilds have found the abandoned '+((BMETA[b.tp]&&BMETA[b.tp].label)||'tower')+' — vines through the shattered glass, moss over the wreck — though its rooms can still be walked.');
 }else if(stage===3){
  b.decay=3;
  for(let y=b.y;y<b.y+b.h;y++)for(let x=b.x;x<b.x+b.w;x++)if(bld[idx(x,y)]===b.i)setSolid(x,y,S_RUIN);
  if(interior&&interior.b===b)exitInterior();   // kick the player out if they were inside as it fell
  if(!quiet)tale([],'At last the old '+((BMETA[b.tp]&&BMETA[b.tp].label)||'tower')+' gave way — its floors pancaking down into a heap of rubble.');
 }
}
// a town whose people are all gone falls into ruin. Its LARGE buildings become
// standing, enterable ruins (recently-derelict cyber shells or nature-reclaimed
// ancient ones) — the playable crawl now lives inside them, not in a dungeon —
// while its small dwellings tumble to rubble.
function makeRuin(v){
 ...
 for(const b of bs){
  if(STAGED_TP[b.tp])decayBuilding(b,chance(.5)?1:2,true);   // a playable tower-ruin
  else makeAncientRuin(b);                                    // small homes → rubble
 }
 tale([], v.name+' has fallen silent — its people gone, its lanes surrendered to the vines. Only the ruins remain now: gutted towers where the lights still flicker, and the wilds pouring in through the rest.',true);
 return null;
}
 ...
 switch(t.k){
  ...
  case 'gohome':{
   ...
   if(t.why==='eat'){
    ...
    // step inside to sleep: they leave the overworld and turn up in the interior
    if(p.home&&!p.home.gone&&ENTER_TP[p.home.tp]&&bsolid(p.home))p.inside=p.home;
   }else p.task=null;
   break;
  }
  case 'build':{
...
function die(p,cause){
 if(p.dead)return;
 p.dead=true;p.task=null;p.path=null;p.sleeping=false;p.inside=null;
...
function dailyTick(){
 ...
 decayFarms();             // untended plots dissolve back into plain terrain
 rebuildCityLinks();       // skybridges & tunnels between the city's great buildings
 ...
 // relics surface as the age decays; the forest slowly reclaims stray tech
 {
  ...
  // common finds the Sage never came for settle back into the earth over time,
  // so uncollected treasure can't clutter the map (relics linger — they're rare)
  if(salvage.length>16){ for(let i=salvage.length-1;i>=0;i--){ const s=salvage[i];
   if(s.kind==='relic'||s.relic)continue;
   if(simMin-(s.born||0)>4*DAY && dist2(s.x,s.y,hero.x,hero.y)>(22*TILE)**2 && chance(.5))salvage.splice(i,1); } }
 }
 // fauna wax in green ages and thin out toward the waste
 {
  ...
  // hard ceiling so they can't overshoot without bound at high speed
  let over=animals.length-(at+8);
  for(let i=animals.length-1;i>=0&&over>0;i--){if(!animals[i].tame){animals.splice(i,1);over--;}}
 }
 // flyers likewise — flocks arrive on the wing, thin out toward the waste
 {
  const ft=flyerTarget();
  if(flyers.length<ft){ if(flyers.length+9<=ft+6&&chance(.35))spawnFlock(); else if(chance(.6))spawnFlyer(); }
  else if(flyers.length>ft+6&&chance(.2)){const fl=flyers[ri(0,flyers.length-1)];if(fl)fl.dead=true}
  if(flyers.length>ft+10)flyers.length=ft+10;   // firm ceiling on wings on screen
 }
...
function updatePerson(p,dt){
 ...
 if(p.inside&&(p.inside.gone||p.inside.ruined))p.inside=null;   // their building fell while they slept
 if(p.sleeping){
  p.energy=clamp(p.energy+(p.home&&!p.home.gone?0.18:0.10)*dt,0,100);
  const t=tod();
  if(p.energy>=99||(t>0.25&&t<0.85&&p.energy>62)){p.sleeping=false;p.inside=null;p.thinkT=simMin+rf(1,4)}   // waking steps them back outside
 }else if(p.inside){p.inside=null}    // any non-sleeping indoor state ends: back to the overworld
 else{
...
function departExpedition(exp){
 ...
 for(const p of exp.members){
  if(p.dead)continue;
  p.inDungeon=true;p.task=null;p.path=null;p.moving=false;p.sleeping=false;p.inside=null;
  d.inside.add(p.id);
  emote(p,'🔦');
 }
...
function nearestSettler(mx,my,filter){
 let best=null,bd=1e18;
 for(const p of people){
  if(p.dead||p.inDungeon||p.inside||(filter&&!filter(p)))continue;   // indoors is safe from the dark
...
function heroKillMonster(m){
 ...
 if(m.boss){
  const ups=heroGainXp(30); if(ups>0)toast('🌟 The Sage reaches level '+Hero.level);
  Hero.renown=(Hero.renown||0)+5; heroGold+=60; heroGems+=ri(2,4); Hero.maxHp++; Hero.hp=Hero.maxHp;
  grantHeroRelic(pick(RELICS));
  toast('👑 '+m.name+' is slain!','card');
  tale([],'The Sage struck down '+m.name+'. The sky seemed to lift. Somewhere, a held breath let go.',true);
  return;
 }
 if(m.champion){
  const ups=heroGainXp(14); if(ups>0)toast('🌟 The Sage reaches level '+Hero.level);
  Hero.renown=(Hero.renown||0)+3; heroGold+=30; heroGems+=1;
  toast('⚔ '+m.name+' is defeated!','card');
  tale([],'The Sage cast down '+m.name+', {villain}\'s herald. The town takes heart.'.replace('{villain}',campaign?campaign.villain:'the enemy'),true);
  return;
 }
...
function campTick(dt){
 ...
 for(const m of monsters){
  if(m.boss||m.champion||m.hunt||m.camp||m.target||simMin-m.born<0.4*DAY)continue;
  let joined=false;
...
function drawAnimal(c,a,t){
 ...
 if(a.questBeast){ const w=20,yy=py-20; c.fillStyle='rgba(10,6,12,0.7)';c.fillRect(px-w/2-1,yy-1,w+2,4); c.fillStyle='#d24a5a';c.fillRect(px-w/2,yy,w*clamp(a.hp/a.maxhp,0,1),3); c.font='6.5px Georgia';c.textAlign='center';c.fillStyle='#ffd0c0';c.fillText(a.name,px,yy-3); }
...
function villagePool(v){ let wood=v.stock.wood||0, stone=v.stock.stone||0; const seen=new Set();
 ...
 return {wood,stone,parts:v.stock.parts||0}; }   // 'parts' are factory-milled and pooled only at the town stores
function spendPool(v,cost){ let nw=cost.wood||0,ns=cost.stone||0;
 v.stock.parts=Math.max(0,(v.stock.parts||0)-(cost.parts||0));
 take(v.stock);
...
function developVillage(v){
 ...
 const afford=(tp)=>{const c=COST[tp];return pool.wood>=(c.wood||0)&&pool.stone>=(c.stone||0)&&pool.parts>=(c.parts||0)};
 ...
 if(homeless>vacant+1 || (dev>=2 && shelters>=3)){
  // a factory city with milled parts on hand raises an arcology — the grandest home of all
  if(dev>=4&&has('factory')>=1&&has('arcology')<Math.max(1,dev-3)&&(v.stock.parts||0)>=(COST.arcology.parts||0)){ if(commission('arcology'))return; }
...
// a plot no living town still works goes fallow and, after a few days, the wild
// green takes it back — the rows dissolve into ordinary empty terrain
function decayFarms(){
 if(!farmTiles.size)return;
 for(const i of Array.from(farmTiles)){
  const x=i%W, y=(i/W)|0;
  let tended=false;
  for(const v of villages){ const cl=v.claim; if(!cl)continue;
   if(x>=cl.x0&&x<=cl.x1&&y>=cl.y0&&y<=cl.y1&&villageMembers(v).length){tended=true;break} }
  if(tended){ if(farmAband.size)farmAband.delete(i); continue; }
  const n=(farmAband.get(i)||0)+1;
  if(n>=3){ farmGrid[i]=0; farmSp[i]=-1; farmTiles.delete(i); farmAband.delete(i); markMod(i); }
  else farmAband.set(i,n);
 }
}
const SAY={
...
let surfPals=null,surfStyle=null,surfSeedN=1,surfMasks=null,surfEra=null,surfSkin=null;
...
function bakeSurfaceTiles(){
 ...
 surfPals=terPals[0];
 // the "skin" the Understory dungeons borrow so they render in THIS world's ground &
 // rock palette/texture instead of their own — one look across surface & dungeon
 surfSkin={grass:KF[0].grass,dirt:KF[0].dirt,styleSeed:'surface-'+seed+'-'+tileSalt,seedN:surfSeedN,theme:worldTheme};
 surfEra=eraState();
...
const BSTYLE={
 ...
 arcology:{wall:'#3f4d63',edge:'#232d3e',roof:'#6fa8d8',win:'#cdefff'},
};
// how modern a building reads scales with its town's development: a young town
// builds plain concrete; a grown city sheathes the same tower in neon glass
function buildingFutur(b){ const v=b.vid!=null?villages.find(x=>x.id===b.vid):null; return clamp(((v?v.dev:2)-1)/4,0,1); }
function drawTallBuilding(c,b){
 ...
 if(arc){
  ...
  const midTop=byTop+bh*0.42, tw=w*0.66, tx=px+(w-tw)/2, fill=c.fillStyle;
  c.fillRect(px,midTop,w,py+h-midTop);
  c.fillStyle=fill;c.fillRect(tx,byTop,tw,midTop-byTop+2);
  c.fillStyle=st.edge;c.fillRect(px+w-3,midTop,3,py+h-midTop);c.fillRect(tx+tw-3,byTop,3,midTop-byTop);
 }else{
  c.fillRect(px,byTop,w,bh);
  c.fillStyle=st.edge;c.fillRect(px+w-3,byTop,3,bh);
 }
 c.fillStyle='rgba(255,255,255,0.06)';c.fillRect(px,byTop,2,bh);
 c.fillStyle=st.roof;c.fillRect(px-1,byTop-3,w+2,4);
 // storeys of windows — a lit grid, bluer & brighter with modernity, some dark
 const cols=Math.max(2,Math.round(w/6)), gapX=(w-4)/cols, seed=(b.id*2654435761)>>>0;
 const arcInset=arc?(w-w*0.66)/2:0;
 for(let r=0;r<stories;r++){const wy=byTop+4+r*STORY, taper=(arc&&r<stories*0.58);
...
let cityLinks=[];
function towerTopY(b){ return b.y*TILE - (BMETA[b.tp]?(BMETA[b.tp].stories||1):1)*9 + 5; }
function rebuildCityLinks(){
 const megs=buildings.filter(b=>!b.gone&&!b.ruined&&b.done&&MEGA_TP[b.tp]);
 const links=[],cnt=new Map();
 for(let i=0;i<megs.length;i++)for(let j=i+1;j<megs.length;j++){
  const a=megs[i],b=megs[j];
  if(a.vid==null||a.vid!==b.vid)continue;
  const ax=a.x+a.w/2,ay=a.y+a.h/2,bx=b.x+b.w/2,by=b.y+b.h/2;
  if(Math.hypot(ax-bx,ay-by)>11)continue;
  if((cnt.get(a)||0)>=2||(cnt.get(b)||0)>=2)continue;
  const kind=((a.id+b.id)%3===0&&(a.tp==='arcology'||b.tp==='arcology'))?'tunnel':'bridge';
  links.push({a,b,kind});cnt.set(a,(cnt.get(a)||0)+1);cnt.set(b,(cnt.get(b)||0)+1);
 }
 cityLinks=links;
}
function drawTunnel(c,a,b){
 const ax=(a.x+a.w/2)*TILE,ay=(a.y+a.h)*TILE-3,bx=(b.x+b.w/2)*TILE,by=(b.y+b.h)*TILE-3;
 c.lineCap='round';
 c.strokeStyle='rgba(12,14,20,0.85)';c.lineWidth=7;c.beginPath();c.moveTo(ax,ay);c.lineTo(bx,by);c.stroke();
 c.strokeStyle='rgba(60,66,80,0.9)';c.lineWidth=5;c.beginPath();c.moveTo(ax,ay);c.lineTo(bx,by);c.stroke();
 c.strokeStyle='rgba(120,220,255,0.5)';c.lineWidth=1;c.setLineDash([3,4]);c.beginPath();c.moveTo(ax,ay);c.lineTo(bx,by);c.stroke();c.setLineDash([]);
 c.lineCap='butt';
}
function drawSkybridge(c,a,b,t){
 const ax=(a.x+a.w/2)*TILE,ay=towerTopY(a)+6,bx=(b.x+b.w/2)*TILE,by=towerTopY(b)+6;
 // support struts down from each end
 c.strokeStyle='rgba(30,36,48,0.6)';c.lineWidth=1.5;
 c.beginPath();c.moveTo(ax,ay);c.lineTo(ax,(a.y+a.h)*TILE-6);c.moveTo(bx,by);c.lineTo(bx,(b.y+b.h)*TILE-6);c.stroke();
 // the deck
 c.lineCap='round';
 c.strokeStyle='rgba(0,0,0,0.28)';c.lineWidth=6;c.beginPath();c.moveTo(ax,ay+2);c.lineTo(bx,by+2);c.stroke();
 c.strokeStyle='#6b7890';c.lineWidth=4.5;c.beginPath();c.moveTo(ax,ay);c.lineTo(bx,by);c.stroke();
 c.strokeStyle='#95a6bf';c.lineWidth=1.5;c.beginPath();c.moveTo(ax,ay-1.6);c.lineTo(bx,by-1.6);c.stroke();
 // running neon lights along the rail
 const len=Math.hypot(bx-ax,by-ay),n=Math.max(2,(len/6)|0);
 for(let k=0;k<n;k++){const f=(k+((t*0.02)%1))/n,lx=ax+(bx-ax)*f,ly=ay+(by-ay)*f-1.6;
  c.fillStyle='rgba(125,227,255,'+(0.5+0.4*Math.sin(t*0.01+k))+')';c.fillRect(lx-0.5,ly-0.5,1.4,1.4);}
 c.lineCap='butt';
}
...
function drawMonster(c,m,t){
 const px=m.x,py=m.y;
 const boss=m.boss, champ=m.champion, big=boss||champ, bs=boss?1.9:champ?1.4:1;
 const go=c.globalCompositeOperation;c.globalCompositeOperation='screen';
 if(boss){const pul=1+0.12*Math.sin(t*0.006);c.globalAlpha=0.8;c.drawImage(GLOW_RED,px-30*pul,py-30*pul,60*pul,60*pul);c.globalAlpha=1;}
 else if(champ){const pul=1+0.1*Math.sin(t*0.006);c.globalAlpha=0.7;c.drawImage(GLOW_PINK,px-24*pul,py-24*pul,48*pul,48*pul);c.globalAlpha=1;}
 else c.drawImage(GLOW_RED,px-18,py-18,36,36);
 c.globalCompositeOperation=go;
 const spr=surfMon[m.type];
 if(spr&&big){
  c.save();c.translate(px,py);c.scale(bs,bs);
  c.fillStyle='rgba(0,0,0,0.45)';c.beginPath();c.ellipse(0,2,8,3,0,0,7);c.fill();
  const anim=(m.atkAnim>0&&spr.FRAMES.attack)?'attack':'walk';
  CFHelp.drawCreatureSprite(c,spr,0,4,m.dirIdx||0,anim,m.animClock);
  c.restore();
  c.font=(boss?'13px':'10px')+' system-ui';c.textAlign='center';c.fillText(boss?'👑':'⚔',px,py-22*bs+6);
 }else if(spr){
 ...
 if(m.boss||m.champion){
  const w=m.boss?26:20,yy=py-(m.boss?34:28);
  c.fillStyle='rgba(10,6,12,0.7)';c.fillRect(px-w/2-1,yy-1,w+2,4);
  c.fillStyle=m.boss?'#e05a7a':'#c07adf';c.fillRect(px-w/2,yy,w*clamp(m.hp/m.maxhp,0,1),3);
  c.font='6.5px Georgia';c.textAlign='center';c.fillStyle=m.boss?'#ffd0da':'#e6d0ff';c.fillText(m.name,px,yy-3);
 ...
 if(m.em&&m.em.until>performance.now()){c.font='9px system-ui';c.textAlign='center';c.fillText(m.em.g,px,py-((m.boss||m.champion)?40:21))}
}
...
function draw(t){
 ...
 // ground tunnels run beneath the towers (draw before the buildings)
 for(const l of cityLinks){ if(l.kind!=='tunnel')continue; if(l.a.gone||l.a.ruined||l.b.gone||l.b.ruined)continue; drawTunnel(ctx,l.a,l.b); }
 for(const b of vis)drawBuilding(ctx,b,nf);
 // skybridges arc over everything (draw after the buildings, at rooftop height)
 for(const l of cityLinks){ if(l.kind!=='bridge')continue; if(l.a.gone||l.a.ruined||l.b.gone||l.b.ruined)continue; drawSkybridge(ctx,l.a,l.b,t); }
 for(const d of dungeons){
  if(d.x<vx0||d.x>vx1||d.y<vy0||d.y>vy1)continue;
  drawDungeon(ctx,d,t);
 }
 ...
 if(!interior){
  let obj=campaignObjectivePx();
  if(!obj){const tq=trackedQuestObj();obj=tq?questObjective(tq):null;}
  if(obj){
   ctx.setTransform(dpr,0,0,dpr,0,0);
 ...
 // boss health bar (screen space) — the campaign's tyrant, front and centre
 if(campaign&&!campaign.done&&campaign.boss&&monsters.indexOf(campaign.boss)>=0){
  const b=campaign.boss;
  ctx.setTransform(dpr,0,0,dpr,0,0);
  const bw=Math.min(360,cw-48), bx=(cw-bw)/2, by=Math.round(ch*0.055);
  ctx.fillStyle='rgba(10,6,12,0.72)';rr(ctx,bx-3,by-3,bw+6,16,5);ctx.fill();
  ctx.fillStyle='rgba(40,10,20,0.9)';ctx.fillRect(bx,by,bw,10);
  const frac=clamp(b.hp/b.maxhp,0,1);
  const g=ctx.createLinearGradient(bx,0,bx+bw,0);g.addColorStop(0,'#e05a7a');g.addColorStop(1,'#ff8a5a');
  ctx.fillStyle=g;ctx.fillRect(bx,by,bw*frac,10);
  ctx.fillStyle='#ffd7e0';ctx.font='bold 11px Georgia';ctx.textAlign='center';ctx.textBaseline='middle';
  ctx.fillText('👑 '+b.name,cw/2,by+5);ctx.textBaseline='alphabetic';
 }
...
const ENTER_TP={home:1,shelter:1,biz:1,apartment:1,warehouse:1,venue:1,factory:1,highrise:1,arcology:1};
...
function makeInterior(b){
 ...
 if(b.tp==='biz'){
  ...
  put(dcx-1,gh-3,2,1,'rug',0);
 }else if(b.tp==='arcology'){
  // a vertical city: lift core, glowing terminals, plazas of seats & hydroponic greens
  put(midx-1,1,2,1,'lift',1);
  put(1,1,1,1,'terminal',1);put(gw-2,1,1,1,'terminal',1);
  put(1,midy,1,1,'potted',1);put(gw-2,midy,1,1,'potted',1);
  for(let y=2;y<gh-2;y+=3)for(let x=2;x<gw-2;x+=4){if(x===dcx||x===dcx-1)continue;put(x,y,1,1,rng()<.5?'seat':'terminal',1);}
  put(gw-3,gh-3,2,1,'machine',1);
  put(dcx-1,gh-3,2,1,'rug',0);
 ...
 const occSrc=people.filter(o=>o.inside===b&&!o.dead);
 const cap=mega?(b.tp==='arcology'?6:4):3;
 for(const o of occSrc.slice(0,cap)){
...
function renderInteriorFP(){
 ...
 let dd=(hero.face||0)-sfpYaw; while(dd>Math.PI)dd-=6.283185; while(dd<-Math.PI)dd+=6.283185; sfpYaw+=dd*0.25;
...
function setSurfaceFP(on){
 ...
 if(sfpActive){ sfpYaw=hero.face||0; FPView.mount($('cvFP')); FPView.resize(cw,ch); sfpLW=cw; sfpLH=ch;
   if(ui)ui.classList.remove('hidden'); if(btn)btn.classList.add('on'); if(interior)renderInteriorFP(); }
 else { if(ui)ui.classList.add('hidden'); if(btn)btn.classList.remove('on'); }
}
function toggleSurfaceFP(){ setSurfaceFP(!sfpActive); }
function surfaceFPEligible(){ const sui=$('sui'); return !!interior && sui && !sui.classList.contains('hidden'); }
...
function drawInterior(t){
 ...
 ctx.imageSmoothingEnabled=false;
 if(intr.bg){
  // the floor & walls, painted from the OVERWORLD's own ground/rock texture, so the
  // interior is drawn by the same engine as everything outside
  ctx.drawImage(intr.bg,0,0);
  // a light shade on the wall tiles for depth, and a decay wash to keep the mood
  ...
  for(let y=0;y<intr.gh;y++)for(let x=0;x<intr.gw;x++){
   const px=x*TILE,py=y*TILE,v=(hash2(x*2+5,y*3+1)*14)|0, base=U.hexToRgb(intr.floor);
   ctx.fillStyle='rgb('+(base[0]+v-4)+','+(base[1]+v-4)+','+(base[2]+v-4)+')';ctx.fillRect(px,py,TILE,TILE);
   ctx.fillStyle='rgba(0,0,0,0.14)';ctx.fillRect(px,py+TILE-1,TILE,1);
  }
  const isDoorF=(x,y)=>y===intr.gh-1&&intr.doorGap.includes(x);
  for(let y=0;y<intr.gh;y++)for(let x=0;x<intr.gw;x++){
   if(x!==0&&y!==0&&x!==intr.gw-1&&y!==intr.gh-1)continue;
   if(isDoorF(x,y))continue;
 ...
 for(const o of intr.occupants)ds.push({y:o.y,f:()=>{
  ...
  if(o.sprite)CFHelp.drawCreatureSprite(ctx,o.sprite,0,3,o.dir,'walk',o.moving?o.animClock:0);
...
function updateContextButtons(){
 ...
 if(interior){
  ...
  // talk to a resident you're standing beside
  talkTarget=null;
  if(!inDialog){
   let bd=(TILE*1.5)**2;
   for(const o of interior.occupants){ if(!o.pid)continue; const d=dist2(o.x,o.y,hero.x,hero.y); if(d<bd){const pr=allById.get(o.pid); if(pr&&!pr.dead){bd=d;talkTarget=pr}} }
  }
  const tbi=$('sTalkBtn');
  tbi.style.display=talkTarget?'block':'none';
  if(talkTarget)tbi.textContent='💬 TALK — '+talkTarget.name;
  return;
 }
 ...
 if(!hero.down&&!inDialog&&speedIdx<=1){
  let bd=44*44;
  for(const p of people){
   if(p.dead||p.inDungeon||p.inside||p.age<8)continue;
...
/* ---- zoom buttons: explicit +/- so pinch isn't needed (frees swipe-to-attack) ---- */
{
 ...
 const sc=$('storyCard'); if(sc)sc.addEventListener('pointerdown',e=>{e.preventDefault();closeStoryCard();});
}
...
// helper: drop a finished, un-owned building of the given type at (x,y)
function placeDoneBuilding(tp,x,y){
 const[w,h]=SZ[tp];
 const b={i:buildings.length,id:nextId++,tp,x,y,w,h,prog:NEED[tp],need:NEED[tp],done:true,gone:false,
 ...
 buildings.push(b);
 for(let yy=b.y;yy<b.y+h;yy++)for(let xx=b.x;xx<b.x+w;xx++){bld[idx(xx,yy)]=b.i;setSolid(xx,yy,S_HOUSE);}
 return b;
}
// the world remembers towers that fell before you came: it opens already dotted with
// the RUINS OF LARGE BUILDINGS — some only recently abandoned (stage-1 cyber shells,
// still lit by dying signs), some long ago (stage-2, nature reclaiming them). These
// are standing, enterable, playable ruins — NOT dungeons.
function seedAncientRuins(n){
 const bigTypes=['apartment','warehouse','factory','highrise','arcology','venue'];
 for(let k=0;k<n;k++){
  const stage = (k%2===0)?1:2;               // alternate so the world holds BOTH recent & ancient tower-ruins
  const tp = pick(bigTypes), [w,h]=SZ[tp];
  // an open plot away from any living town (findSpot reliably clears a footprint)
  let spot=null;
  for(let t=0;t<40&&!spot;t++){
   const ax=ri(6,W-6), ay=ri(6,H-6);
   if(villages.some(v=>dist2(ax,ay,v.cx,v.cy)<15*15))continue;
   const s=findSpot(ax,ay,w,h,null);
   if(s&&!villages.some(v=>dist2(s[0],s[1],v.cx,v.cy)<13*13))spot=s;
  }
  if(!spot)continue;
  const b=placeDoneBuilding(tp,spot[0],spot[1]);
  decayBuilding(b,1,true);                    // recent-cyber (silent)
  if(stage===2)decayBuilding(b,2,true);       // …weathered on into an ancient ruin
  // a scatter of tumbled small homes around the fallen tower — the old district
  for(let i=0;i<ri(1,3);i++){
   const st=pick(['home','home','apartment']), [sw,sh]=SZ[st];
   const sp=findSpot(clamp(spot[0]+ri(-5,5),2,W-3),clamp(spot[1]+ri(-5,5),2,H-3),sw,sh,null); if(!sp)continue;
   const rb=placeDoneBuilding(st,sp[0],sp[1]);
   makeAncientRuin(rb);                        // small dwellings just go to rubble
  }
 }
}
...
/* ================= the Story: a procedural, escalating RPG campaign =================
   ...
   its towns, its troubled folk, its buried Understories — and threads a linear main
...
/* ---- ten more quest kinds, each keyed to a different living system ---- */
const BEAST_N=['Stalker','Fang','Prowler','Gloom-Cat','Fen-Hound','Shrike','Maw','Howler'];
function beastName(){ return 'the '+((Lore.active&&Lore.name(3,7))||pick(['Grey','Pale','Old','Black','Hollow','Red']))+' '+pick(BEAST_N); }
const CHAMPIONS={
 fantasy:['the Iron Revenant','the Oathbreaker','the Bone Herald','the Ash Knight'],
 cyberpunk:['the Deadhand Proxy','the Null Warden','the Scrap Tyrant','the Grey Envoy'],
 modern:['the Pale Foreman','the Hollow Sheriff','the Rust Preacher','the Silent Marshal'],
};
function championName(){ return pick(CHAMPIONS[worldTheme]||CHAMPIONS.fantasy); }
// escort a stranded soul home
function genEscort(H){
 const home=homeVillage(); if(!home)return null;
 const pool=people.filter(p=>!p.dead&&!p.inDungeon&&p.age>=16&&p.vid!==home.id);
 const p=pool.length?pick(pool):null; if(!p)return null;
 return {k:'escort',targetId:p.id,figName:p.name,dest:[Math.round(home.cx),Math.round(home.cy)],stage:0,
  title:'The Long Road Home', intro:p.name+' was caught out in the wild when the shadow fell, and dares not cross the dark alone. Find them, and see them safe back to {home}.',
  desc:'Escort '+p.name+' safely home to {home}.'};
}
// hunt a great wild beast
function genBeast(H){
 const nm=beastName(); let spot=null;
 if(H.ruins.length)spot=[pick(H.ruins).x,pick(H.ruins).y]; if(!spot)spot=farCornerTile(hero);
 if(!spot)return null;
 return {k:'beast',beastName:nm,spot,
  title:'The Hunt for '+nm, intro:'A great beast the folk name '+nm+' has come down from the wild places, emboldened by {villain}, and takes livestock and stragglers alike. Track it down and put it to rest.',
  desc:'Hunt down '+nm+'.'};
}
// befriend a companion
function genTame(H){
 return {k:'tame',
  title:'An Omen on Four Legs', intro:'The old belief holds that a wild thing befriended in an age of dread is a sign the land has not given up. Win a creature\'s trust, Sage — give {home} an omen to rally behind. (Loiter beside a calm animal until it trusts you.)',
  desc:'Befriend a wild animal companion.'};
}
// recover a lost relic
function genRelic(H){
 let spot=null,loc='the wastes';
 if(H.ruins.length){const r=pick(H.ruins);spot=[r.x,r.y];loc=r.name;}
 else if(H.dungeons.length){const d=pick(H.dungeons);spot=[d.x,d.y];loc='the ground near '+d.name;}
 else {const s=farCornerTile(hero);if(s)spot=s;}
 if(!spot)return null;
 return {k:'relic',spot,loc,
  title:'The Lost Relic', intro:'A relic of the old machine-world lies half-buried at '+loc+' — the kind of thing that might turn a hopeless fight. Recover it before {villain}\'s creatures do.',
  desc:'Recover the lost relic from '+loc+'.'};
}
// raise gold to fund the town's defence
function genTribute(H){
 return {k:'tribute',need:0,
  title:'A King\'s Ransom', intro:'The free-companies will fight for {home} — but only for coin. Dig {need} gold from the hills and the rubble, and buy the town its swords.',
  desc:'Raise {need} gold for {home}\'s defence.'};
}
// hold a night-vigil against waves
function genVigil(H){
 const home=homeVillage(); if(!home)return null;
 const spot=[clamp(Math.round(home.cx)+ri(-6,6),3,W-3),clamp(Math.round(home.cy)+ri(-6,6),3,H-3)];
 return {k:'vigil',spot,
  title:'The Long Night', intro:'Tonight the Understory empties itself against {home}. Take the watch at the town\'s edge and hold your ground until the assault breaks — no matter how many come.',
  desc:'Hold the watch at {home} until the assault breaks.'};
}
// find the travelling merchant
function genTrade(H){
 return {k:'trade',
  title:'The Pedlar\'s Bargain', intro:'A travelling pedlar with a cart of oddments and older news has been seen on the roads. Find them — they may carry something the town sorely needs.',
  desc:'Seek out the travelling merchant.'};
}
// rekindle a festival
function genRite(H){
 const home=homeVillage(); if(!home)return null;
 return {k:'rite',festVid:home.id,
  title:'Rekindle the Feast', intro:'Fear has smothered the joy out of {home}. Rekindle it — call the lanterns up over the plaza, and stand among the folk as they remember how to sing.',
  desc:'Hold a festival in {home} and join it.'};
}
// walk the old road of landmarks
function genPilgrimage(H){
 const stops=[];
 for(const s of shrines)stops.push([(s.x/TILE)|0,(s.y/TILE)|0,'the Sage-shrine']);
 for(const r of H.ruins)stops.push([r.x,r.y,r.name]);
 for(const d of H.dungeons)stops.push([d.x,d.y,'the mouth of '+d.name]);
 for(const v of H.villages)stops.push([Math.round(v.cx),Math.round(v.cy),v.name]);
 if(stops.length<3)return null;
 shuffle(stops); const chosen=stops.slice(0,3);
 return {k:'pilgrimage',stops:chosen,si:0,
  title:'The Old Road', intro:'The elders say the land\'s strength sleeps in its old places and must be woken in turn. Walk the old road — '+chosen.map(s=>s[2]).join(', then ')+' — and wake it.',
  desc:'Walk the old road: visit three sacred places.'};
}
// defeat a rival champion (a mini-boss ahead of the finale)
function genChampion(H){
 const nm=championName(), home=homeVillage();
 const c=home?[Math.round(home.cx),Math.round(home.cy)]:[(hero.x/TILE)|0,(hero.y/TILE)|0];
 return {k:'champion',champName:nm,spot:c,
  title:'The Champion '+nm, intro:nm+', a herald sent ahead by {villain}, has come to break {home}\'s will before its master arrives. Meet it in the open and cast it down.',
  desc:'Defeat '+nm+'.'};
}
function spawnChampion(name,c){
 let s=null;
 for(let r=6;r<=16&&!s;r++){ const cand=nearOpen(clamp(c[0]+ri(-r,r),2,W-2),clamp(c[1]+ri(-r,r),2,H-2)); if(cand&&dist2(cand[0]*TILE,cand[1]*TILE,hero.x,hero.y)>(6*TILE)**2)s=cand; }
 if(!s)s=nearOpen(c[0],c[1])||[W>>1,H>>1];
...
// spawn a great wild beast (a boosted predator) for a hunt-the-beast chapter
function spawnBeast(name,spot){
 const a=spawnAnimal('wolf'); if(!a)return null;
...
// gently pull an escorted villager along behind the Sage
function escortStep(p,dt){
 p.thinkT=simMin+120; if(p.task&&p.task.k!=='fight')p.task=null; p.sleeping=false;
 const dx=hero.x-p.x,dy=hero.y-p.y,d=Math.hypot(dx,dy)||1;
 if(d>1.6*TILE){ const spd=Math.min(d,90*dt);
  const nx=p.x+dx/d*spd, ny=p.y+dy/d*spd;
  if(walkable((nx/TILE)|0,(p.y/TILE)|0))p.x=nx;
  if(walkable((p.x/TILE)|0,(ny/TILE)|0))p.y=ny;
...
function campaignNoteDescend(d){
 ...
 const ch=campaign.chapters[campaign.idx];
 if(ch&&ch.k==='dungeon')ch._descended=true;   // any descent answers the call
}
function campaignTick(dt){
 ...
 if(ch.k==='reach'){ if(ch.mark&&dist2(hero.x,hero.y,ch.mark[0]*TILE+TILE/2,ch.mark[1]*TILE+TILE/2)<(2.8*TILE)**2)done=true; }
  ...
  else if(ch.stage===0){ ch.mark=[(p.x/TILE)|0,(p.y/TILE)|0]; if(dist2(hero.x,hero.y,p.x,p.y)<(2.6*TILE)**2){ ch.stage=1; p.escort=true; emote(p,'🙏'); toast('Lead '+p.name+' home. Stay close.'); updateStoryBanner(); } }
  else { escortStep(p,dt); ch.mark=ch.dest;
   const atDest=dist2(p.x,p.y,ch.dest[0]*TILE+TILE/2,ch.dest[1]*TILE+TILE/2)<(3.2*TILE)**2;
   const led=dist2(hero.x,hero.y,ch.dest[0]*TILE+TILE/2,ch.dest[1]*TILE+TILE/2)<(3.2*TILE)**2&&dist2(p.x,p.y,hero.x,hero.y)<(5*TILE)**2;
   if(atDest||led){ p.escort=false; emote(p,'🏠'); done=true; } }
 }
 else if(ch.k==='beast'){ if(!ch._beast||animals.indexOf(ch._beast)<0)done=true; else ch.mark=[(ch._beast.x/TILE)|0,(ch._beast.y/TILE)|0]; }
 else if(ch.k==='tame'){ if(companion)done=true; }
 else if(ch.k==='relic'){ if(Hero.relics.length>ch.base)done=true; }
 else if(ch.k==='tribute'){ ch.prog=Math.max(0,heroGold-ch.base); if(ch.prog>=ch.need)done=true; }
 else if(ch.k==='vigil'){
  const near=dist2(hero.x,hero.y,ch.mark[0]*TILE+TILE/2,ch.mark[1]*TILE+TILE/2)<(7*TILE)**2;
  if(near){ ch.vigilT=(ch.vigilT||0)+dt; ch.spawnT-=dt;
   // only send the next wave once the current one is thinning, so it never snowballs
   const abroad=monsters.reduce((a,m)=>a+(m.hunt&&dist2(m.x,m.y,ch.mark[0]*TILE+TILE/2,ch.mark[1]*TILE+TILE/2)<(10*TILE)**2?1:0),0);
   if(ch.spawnT<=0&&abroad<3){ ch.spawnT=rf(4,6); spawnHuntPack(ch.mark[0]+ri(-5,5),ch.mark[1]+ri(-5,5),1+(ch.vigilT>(ch.dur||12)*0.5?1:0)); } }
  ch.prog=Math.min(1,(ch.vigilT||0)/(ch.dur||12));
  if((ch.vigilT||0)>=(ch.dur||12))done=true;
 }
 else if(ch.k==='trade'){
  if(!merchant){ if(!spawnMerchant()&&!merchant){done=true;} if(merchant)merchant.state='stalled'; }
  if(merchant){ ch.mark=[merchant.tx,merchant.ty];
   if(dist2(hero.x,hero.y,merchant.x,merchant.y)<(3*TILE)**2){ done=true; heroGold+=20; emote2Hero('🤝'); toast('🛒 The pedlar shares news and a little coin.'); } }
 }
 else if(ch.k==='rite'){
  const v=villages.find(x=>x.id===ch.festVid);
  if(!v){ done=true; }
  else { ch.mark=[Math.round(v.cx),Math.round(v.cy)];
   if(dist2(hero.x,hero.y,v.cx*TILE,v.cy*TILE)<(4.5*TILE)**2){ done=true; hero.festiveUntil=simMin+120; toast('🎉 You rekindle the feast, and '+v.name+' remembers how to hope.'); } }
 }
 else if(ch.k==='pilgrimage'){
  const st=ch.stops[ch.si]; if(st){ ch.mark=[st[0],st[1]];
...
function campaignObjectivePx(){
 ...
 if((ch.k==='gather'||ch.k==='tribute')){ const r=nearestRockPx(hero.x,hero.y); if(r)return r; }
 if(ch.k==='beast'){ if(ch._beast&&animals.indexOf(ch._beast)>=0)return[ch._beast.x,ch._beast.y]; }
 if(ch.k==='champion'){ if(ch._champ&&monsters.indexOf(ch._champ)>=0)return[ch._champ.x,ch._champ.y]; }
 if(ch.k==='escort'&&ch.stage===0){ const p=allById.get(ch.targetId); if(p&&!p.dead)return[p.x,p.y]; }
 if(ch.k==='trade'&&merchant)return[merchant.x,merchant.y];
 if(ch.k==='tame'){ let a=null,bd=1e18; for(const aa of animals){if(aa.dead||aa.tame||aa.temper==='predator')continue;const dd=dist2(aa.x,aa.y,hero.x,hero.y);if(dd<bd){bd=dd;a=aa;}} if(a)return[a.x,a.y]; }
 if(ch.mark)return[ch.mark[0]*TILE+TILE/2,ch.mark[1]*TILE+TILE/2];
 return null;
}
/* a named boss: an empowered horror that rises to threaten the home town */
function spawnBoss(){
 const home=homeVillage();
 const c=home?[Math.round(home.cx),Math.round(home.cy)]:[(hero.x/TILE)|0,(hero.y/TILE)|0];
 let s=null;
 for(let r=7;r<=18&&!s;r++){ const cand=nearOpen(clamp(c[0]+ri(-r,r),2,W-2),clamp(c[1]+ri(-r,r),2,H-2)); if(cand&&dist2(cand[0]*TILE,cand[1]*TILE,hero.x,hero.y)>(7*TILE)**2)s=cand; }
 if(!s)s=nearOpen(c[0],c[1])||[W>>1,H>>1];
 const M=MONSTERS.horror;
...
function updateStoryBanner(){
 ...
 if(!campaign){el.classList.add('hidden');return}
 ...
 else if(ch.k==='tribute')prog=' · '+Math.min(ch.prog||0,ch.need)+'/'+ch.need+' gold';
 else if(ch.k==='feud')prog=' · '+(ch.stage===0?'find '+ch.aName:'now find '+ch.bName);
 else if(ch.k==='escort')prog=' · '+(ch.stage===0?'find '+ch.figName:'lead '+ch.figName+' home');
 else if(ch.k==='vigil')prog=' · '+Math.round((ch.prog||0)*100)+'% held';
 else if(ch.k==='pilgrimage')prog=' · '+((ch.si||0))+'/'+(ch.stops?ch.stops.length:3);
 else if(ch.k==='champion')prog=' · ⚔';
 else if(ch.k==='beast')prog=' · 🐾';
...
return {
 ...
 api:{
  ...
  campaign:()=>{ if(!campaign)return null; const ch=campaign.chapters[campaign.idx];
   return {title:campaign.title,villain:campaign.villain,idx:campaign.idx,chapters:campaign.chapters.length,done:campaign.done,
    ...
    chapter:ch?{k:ch.k,title:fillTokens(ch.title,ch),desc:ch._desc||fillTokens(ch.desc,ch),prog:ch.prog||0,need:ch.need||ch.packN||null,stage:ch.stage,si:ch.si,
     ...
     beastAlive:ch.k==='beast'?(ch._beast&&animals.indexOf(ch._beast)>=0):null,
     champAlive:ch.k==='champion'?(ch._champ&&monsters.indexOf(ch._champ)>=0):null}:null,
  ...
  forceChapterKind:(k)=>{ if(!campaign)return null; const gm={reach:genReach,npc:genNpc,feud:genFeud,bloom:genBloom,hunt:genHunt,guard:genGuard,gather:genGather,dungeon:genDungeon,escort:genEscort,beast:genBeast,tame:genTame,relic:genRelic,tribute:genTribute,vigil:genVigil,trade:genTrade,rite:genRite,pilgrimage:genPilgrimage,champion:genChampion}[k]; if(!gm)return null; const c=gm(campaign.history); if(!c)return null; campaign.chapters.splice(campaign.idx,0,c); activateChapter(); return campaign.chapters[campaign.idx].k; },
  godMode:(on)=>{Hero.maxHp=on?99:5;Hero.hp=Hero.maxHp;return Hero.hp},
  ...
  fpInfo:()=>({active:sfpActive,scene:interior?'interior':'overworld',yaw:+sfpYaw.toFixed(2),pitch:+sfpPitch.toFixed(1),atlasI:!!sfpAtlasI,mat:sfpMatI,sprites:(sfpActive&&interior)?intFPSprites().length:0,fpw:FPView.W,fph:FPView.H}),
  ...
  farmAbandonSet:(radius)=>{ // TEST: convert some open tiles far from villages into farm plots to exercise decay
   let n=0; for(const [x,y] of openTiles){ if(n>=(radius||10))break; if(villages.some(v=>v.claim&&x>=v.claim.x0&&x<=v.claim.x1&&y>=v.claim.y0&&y<=v.claim.y1))continue; const i=idx(x,y); if(farmGrid[i])continue; farmGrid[i]=1;farmSp[i]=0;farmTimer[i]=DAY;farmTiles.add(i);n++; } return farmTiles.size; },
