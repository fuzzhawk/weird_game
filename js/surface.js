...
function makeInterior(b){
 ...
 // continuity: show occupants actually AT the building in the overworld (standing on/
 // adjacent to its footprint), so the inside matches what you saw outside. Big civic
 // buildings pull in nearby townsfolk as workers/visitors so they aren't deserted.
 const atHome=o=>{ const tx=(o.x/TILE)|0,ty=(o.y/TILE)|0;
   return tx>=b.x-1&&tx<b.x+b.w+1&&ty>=b.y-1&&ty<b.y+b.h+1; };
 // a building's own residents are always in (it's their home); prefer to place those
 // actually standing at the footprint, but never leave a lived-in home deserted
 const residents=(b.owners||[]).map(id=>allById.get(id)).filter(o=>o&&!o.dead&&o.sprite);
 const here=residents.filter(atHome);
 let occSrc=here.length?here:residents;
 if(mega&&occSrc.length<2){
  for(const o of people){ if(o.dead||!o.sprite||o.inDungeon||o.age<8||occSrc.includes(o))continue; if(atHome(o))occSrc.push(o); }
 }
 const cap=mega?(b.tp==='arcology'?6:4):3;
 for(const o of occSrc.slice(0,cap)){
  const s=freeSpotInt(intr,rng);
  intr.occupants.push({pid:o.id,name:o.name,sprite:o.sprite,x:s[0]*TILE+TILE/2,y:s[1]*TILE+TILE/2,
