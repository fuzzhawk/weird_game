...
let questStage=0, questKills=0, bloomsGot=0, questVariant='cull';
...
function questLabel(){
  switch(questStage){
    case 0: return '✦ Find '+keeperName;
    case 1:
      if(questVariant==='cull') return `⚔ Unsay the blight — ${questKills}/8`;
      if(questVariant==='blooms') return `❀ Gather night-blooms — ${bloomsGot}/5`;
      return '☠ Refute the heart of this floor';
    case 2: return '✦ Return to '+keeperName;
    case 3:
      if(isFinalFloor()&&cleansedRun) return '⭐ Put to rest — take the way up ◎';
      return isFinalFloor()?'◎ Take the way up':'◎ The way down is open';
  }
  return '';
}
...
function talkToKeeper(){
  const S=SCRIPTS[questVariant];
  switch(questStage){
    case 0:
      openDialog(keeperName,S.intro,()=>{
        questStage=1; questKills=0; bloomsGot=0;
        if(questVariant==='blooms') spawnBlooms();
        if(questVariant==='boss') spawnBoss();
        SFX.quest(); updHud();
      });
      break;
    case 1:{
      const n=questVariant==='cull'?(8-questKills):questVariant==='blooms'?(5-bloomsGot):1;
      openDialog(keeperName,[S.mid.replace('{n}',n)]);
      break;
    }
    case 2:{
      const lines=S.done.slice();
      if(isFinalFloor())lines.push(...FINAL_DONE);
      else lines.push('The way down is open now. The drafts below are rougher.');
      openDialog(keeperName,lines,()=>{
        S.reward();
        SFX.level();
        questStage=3;
        if(isFinalFloor()){ cleansedRun=true; }
        else spawnPortalDown();
        SFX.quest(); updHud();
      });
      break;
    }
    case 3:
      openDialog(keeperName,[IDLE_LINES[(floorIdx+questKills)%IDLE_LINES.length]]);
      break;
  }
}
...
function startFloor(){
  ...
  keeperSprite=null;
  keeperName=KEEPER_NAMES[hashSeed(worldSeed+'/keeper')%KEEPER_NAMES.length];
  questVariant=pickQuestVariant(); questStage=0; questKills=0; bloomsGot=0;
...
function spawnPortalDown(){
  ...
  floater(player.x,player.y-30,'THE WAY DOWN OPENS','#b08fff');
}
...
function onKill(e){
  ...
  if(questStage===1&&questVariant==='cull'){ questKills++; if(questKills>=8){ questStage=2; SFX.quest(); } }
...
function update(dt){
  ...
  // --- spawning ---
  spawnT-=dt;
  const cap = 8 + Math.min(10,player.level) + floorIdx*2 + (questStage>=3?4:0);
  ...
  if(questStage===1&&questVariant==='blooms'){
    for(const b of blooms){
      ...
      if(!b.got && Math.hypot(b.x-player.x,b.y-player.y)<player.r+12){
        ...
        if(bloomsGot>=5){ questStage=2; SFX.quest(); }
        updHud();
      }
    }
  }
  ...
  $('dTalkBtn').style.display=(nearNpc&&!dialogQueue)?'block':'none';
...
function compassTarget(){
  if(questStage===0||questStage===2) return {x:npcs[0].x,y:npcs[0].y,g:'✦'};
  if(questStage===1&&questVariant==='blooms'){ const b=blooms.find(b=>!b.got); if(b) return {x:b.x,y:b.y,g:'❀'}; }
  if(questStage===1&&questVariant==='boss'&&boss) return {x:boss.x,y:boss.y,g:'☠'};
  if(questStage===3){
    if(!isFinalFloor()&&portalDown) return {x:portalDown.x,y:portalDown.y,g:'▼'};
    if(portalUp) return {x:portalUp.x,y:portalUp.y,g:'▲'};
  }
  return null;
}
...
function draw(time){
  ...
  if(dead){
    ...
    ctx.fillText('the Keeper gathers you up…',W/2,H/2+22);
  }
}
...
return {
  ...
  debug:{
    ...
    completeTask(){
      ...
      if(questVariant==='cull'){questKills=8;questStage=2}
      else if(questVariant==='blooms'){for(const b of blooms)b.got=true;bloomsGot=5;questStage=2}
      else if(boss){boss.hp=0;boss.dead=true;onKill(boss)}
      SFX.quest();updHud();
    },
    talk(){talkToKeeper()},
